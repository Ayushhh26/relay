...
//#endregion
//#region src/index.ts
const spacetimedb = schema({ room: table({
	name: "room",
	public: true
}, {
...
const createRoom = spacetimedb.reducer({ title: t.string() }, (ctx, { title }) => {
	for (const existing of ctx.db.room.iter()) if (existing.title === title) return;
	ctx.db.room.insert({
		id: 0n,
		title,
